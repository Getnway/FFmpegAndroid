...
//                commandLine = FFmpegUtil.transformAudio(srcFile, transformFile);
                commandLine = FFmpegUtil.transformMp3("/storage/emulated/0/lansongBox/th.mp3", "/storage/emulated/0/lansongBox/th.m4a");
                break;
            case 1://剪切
...
//                srcFiles.add(appendFile);
                String concatFile = "/storage/emulated/0/lansongBox/1234.m4a";
                List<String> srcFiles = new ArrayList<>();
                for (int i = 101; i <= 112; ++i) {
                    srcFiles.add("/storage/emulated/0/lansongBox/" + i + ".m4a");
                }
                commandLine = FFmpegUtil.concatAudio(srcFiles, concatFile);
                break;
            case 3://混合
                if (!FileUtil.checkFileExist(appendFile)) {
                    return;
